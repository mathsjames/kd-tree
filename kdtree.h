...
namespace kdt
{
  ...
  template <class PointT>
	...
	return true;
      ...
      /** @brief Inserts a new entry into the tree
       */
      void insert(const PointT& newPoint)
      {
	points_.push_back(newPoint);
	if (root_)
	  {
	    insertRecursive(newPoint, root_, 0);
	  }
	else
	  {
	    root_ = new Node();
	...
	if (node1)
      ...
      /** Insert a new point into the tree recursively
       */
      void insertRecursive(const PointT& newPoint, Node* node, int depth)
      {
	const int axis = node->axis;
	...
	if (node->next[dir])
	  {
	    insertRecursive(newPoint, node->next[dir], depth++);
	  }
	else
	  {
